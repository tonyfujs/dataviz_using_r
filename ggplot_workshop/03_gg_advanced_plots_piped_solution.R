...
troops %>%
  ggplot() %>%
  + geom_path(aes(x = long, y = lat, group = group, color = direction))
...
ggplot(aes(x = long, y = lat)) %>%
  + geom_path(data = troops, aes(group = group, color = direction, size = survivors)) %>%
...
ggplot() + 
	geom_path(
		aes(x = long, y = lat, group = group, color = direction, size = survivors), 
		data = troops, 
		lineend = "round") +
	geom_point(
		aes(x = long, y = lat),
		data = cities) +
	geom_text(
		aes(x = long, y = lat-0.03, label = city), size = 4,
		data = cities)
...
p <- p + geom_path(aes(group = group, color = direction, size = survivors),
									 data = troops,
									 lineend = "round")
p <- p + geom_point()
...
ggplot(
	aes(x = long, y = lat), # Replace the XXXX
	data = walmart) +
	geom_point()
